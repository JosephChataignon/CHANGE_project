...
def get_CHANGE_data(data_type):
    if data_type.lower() == "walser":
        ...
        data_dir = '/research_storage/Data_MaxPlanckInstitut/'
        # testing segments selection, for faster execution
        train_folders = ["seg01"]
        test_folders = ["seg71"]
        val_folders = ["seg80"]
        # replace the "segXX" with a path to txt files, using a * wildcard
        # add *01.txt to the path to select just ~1% of files for each segment
        data_folders = []
        for folders_list in [train_folders, test_folders, val_folders]:
            folders_list = [f"{data_dir}output/{seg}/input/{seg}*01.txt" for seg in folders_list]
            data_folders.append(folders_list)

        #return tuple(data_folders)
        return f"{data_dir}output/seg01/input/seg01_1524_00000030.txt", f"{data_dir}output/seg71/input/seg71_206422_00000318.txt", None#f"{data_dir}output/seg80/input/seg80_231690_00000395.txt"
